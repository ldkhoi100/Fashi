...
@section('title', 'Edit size-quantity product')
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-edit"></i> Update quantity-size of
                        product "{{ $id_product->name }}", id: {{ $id_product->id }}</h6>
                </div>

                <div class="card-body">
                    <div class="chart-area" style="height: auto">
                        <form method="post" action="{{ route('product.qtySizePost', $id_product->id) }}"
                            ...
                            <?php $i=0; ?>
                            @foreach ($size_product as $qty)

                            <div class="form-group @error('changeqty{{ $i }}') has-error has-feedback @enderror">

                                <label>{{ $qty->size->name }}</label>

                                <input type="number" min="0" max="10000"
                                    class="form-control @error('changeqty{{ $i }}') is-invalid @enderror"
                                    name="changeqty{{ $i }}" value="{{ $qty->quantity }}" required>
