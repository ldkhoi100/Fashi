                        ...
                        <form method="post" action="{{ route('banner.store') }}" enctype="multipart/form-data"
                            ...
                            <div class="form-group @error('position') has-error has-feedback @enderror">
                                ...
                                <select name="position" id=""
                                    ...
                                    <option value="0" @if(old('position')) {{ "selected" }} @endif>None</option>
                                    <option value="1" @if(old('position')) {{ "selected" }} @endif>First</option>
                                    <option value="2" @if(old('position')) {{ "selected" }} @endif>Second</option>
                                    <option value="3" @if(old('position')) {{ "selected" }} @endif>Third</option>
                                </select>
                            </div>
